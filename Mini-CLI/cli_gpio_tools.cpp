...
// protkesi pin
const uint8_t protectedPins[] = {0, 1, 4, 5};  // tambahkan pin penting lainnya
...
bool isValidPinList(char* str) {
  while (*str) {
    if (!isdigit(*str) && *str != ',') {
      return false; // Kalau bukan digit dan bukan koma -> salah
    }
    str++;
  }
  return true;
...
void split(char* input, uint8_t* output, uint8_t& size, char splitter) {
  ...
  // Cek kalau splitter tidak ditemukan di input
  if (strchr(input, splitter) == NULL) {
    output[0] = atoi(input);  // Langsung konversi seluruh input
    size = 1;
    return;
  }

  // Pisahkan string berdasarkan karakter splitter
